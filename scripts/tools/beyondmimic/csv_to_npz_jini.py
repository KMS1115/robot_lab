...
"""Convert a JiNi-retargeted CSV motion into BeyondMimic NPZ format.
...
parser = argparse.ArgumentParser(description="Replay JiNi motion from a csv file and output it to a npz file.")
...
class ReplayMotionsSceneCfg(InteractiveSceneCfg):
    ...
    robot: ArticulationCfg = JINI_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        if motion.shape[1] != 17:
            raise ValueError(
                f"JiNi motion CSV must have 17 columns (3 base pos + 4 quat + 10 joints). Got {motion.shape[1]}."
            )
