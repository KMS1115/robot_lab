...
@dataclass
class JiNiRoughTrainerCfg(TrainerCfg):
    max_iterations = 2000
    save_interval = 100
    experiment_name = "jini_rough"
    agent_factory = cusrl.ActorCritic.Factory(
        ...
        actor_factory=cusrl.Actor.Factory(
            backbone_factory=cusrl.Mlp.Factory(
                hidden_dims=[256, 256, 128], activation_fn="ELU", ends_with_activation=True
            ),
...
@dataclass
class JiNiFlatTrainerCfg(JiNiRoughTrainerCfg):
    max_iterations = 1200
    experiment_name = "jini_flat"
